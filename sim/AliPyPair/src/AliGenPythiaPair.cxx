...
/* $Id: AliGenPythiaPair.cxx 63851 2013-08-14 07:58:44Z morsch $ */
...
#include "AliGenPythiaPair.h"

ClassImp(AliGenPythiaPair)


AliGenPythiaPair::AliGenPythiaPair():
    AliGenMC(),
    fProcess(kPyCharm),          
    fItune(-1),
    fStrucFunc(kCTEQ5L), 
    ...
    fGenPartonFlavor(22),
    fGenPartonEnergy(50.), 
    fIsGenPartonPair(kTRUE),
    fIsGenUnsignEta(kFALSE),
    fGenSigmaEta(-1.),
    fIsGenFixedEta(kTRUE),
    fGenPartonEtaMin(-1.),
    fGenPartonEtaMax(1.),
    fGenSigmaPhi(-1.),
    fIsGenFixedPhi(kTRUE),
    fGenPartonPhiMin(0.),
    fGenPartonPhiMax(TMath::TwoPi())
{
// Default Constructor
  fEnergyCMS = 5500.;
...
AliGenPythiaPair::AliGenPythiaPair(Int_t npart)
    :AliGenMC(npart),
     fProcess(kPyCharm),          
     fItune(-1),
     fStrucFunc(kCTEQ5L), 
...
void AliGenPythiaPair::SetInteractionRate(Float_t rate,Float_t timewindow)
{
...
void AliGenPythiaPair::GeneratePileup()
{
...
void AliGenPythiaPair::SetPycellParameters(Float_t etamax, Int_t neta, Int_t nphi,
...
void AliGenPythiaPair::SetEventListRange(Int_t eventFirst, Int_t eventLast)
{
...
///////////////////// XIAOMING ZHANG //////////////////////////////////////////
  ...
  if (fGenPartonNumber<=0) fGenPartonNumber = 1;

//if (fUserDebug) {
    cout << "fGenPartonNumber = " << fGenPartonNumber << endl;
    cout << "fGenPartonFlavor = " << fGenPartonFlavor << endl;
    cout << "fGenPartonEnergy = " << fGenPartonEnergy << " (GeV)" << endl << endl;

    cout << "fIsGenPartonPair = " << (fIsGenPartonPair ? "kTRUE" : "kFALSE") << endl;
    cout << "fIsGenUnsignEta  = " << (fIsGenUnsignEta  ? "kTRUE" : "kFSLSE") << endl << endl;

    cout << "fIsGenFixedEta = " << (fIsGenFixedEta ? "kTRUE" : "kFALSE") << endl;
    cout << "fGenSigmaEta   = " << fGenSigmaEta << endl;
    cout << fGenPartonEtaMin << " < eta < " << fGenPartonEtaMax  << endl << endl;

    cout << "fIsGenFixedPhi = " << (fIsGenFixedEta ? "kTRUE" : "kFALSE") << endl;
    cout << "fGenSigmaPhi   = " << fGenSigmaEta << endl;
    cout << fGenPartonPhiMin << " < phi < " << fGenPartonPhiMax  << endl << endl;
//}
...
void AliGenPythiaPair::Generate()
{
// Generate one event
...
//=============================================================================
///////////////////// XIAOMING ZHANG //////////////////////////////////////////
    for (Int_t i=1; i<=fGenPartonNumber; i++) {
...
//    iA = ((i==1) ?  0 : (iC-1));
    ...
    Double_t dEta = 0.;
    if (fIsGenFixedEta)
      dEta = 0.5 * (fGenPartonEtaMin + fGenPartonEtaMax);
    else
      dEta = AliPythiaRndm::GetPythiaRandom()->Uniform(fGenPartonEtaMin, fGenPartonEtaMax);
    Double_t dTheta = CovEtaToTheta(dEta);

    Double_t dPhi = 0.;
    if (fIsGenFixedPhi)
      dPhi = 0.5 * (fGenPartonPhiMin + fGenPartonPhiMin);
    else
      dPhi = AliPythiaRndm::GetPythiaRandom()->Uniform(fGenPartonPhiMin, fGenPartonPhiMax);
    if (dPhi<0.) dPhi += TMath::TwoPi(); if (dPhi>TMath::TwoPi()) dPhi -= TMath::TwoPi();

    fPythia->Py1ent(iA, fGenPartonFlavor, fGenPartonEnergy, dTheta, dPhi);

    if (fUserDebug) cout << "iA = " << iA
                         << ", flavor = " << fGenPartonFlavor
                         << ", energy = " << fGenPartonEnergy
                         << ", theta = "  << dTheta
                         << ", phi = "    << dPhi << endl;

    if (fIsGenPartonPair) {
      if (fGenPartonFlavor!=21) fGenPartonFlavor = -1 * fGenPartonFlavor;

      if (fIsGenUnsignEta) dEta = -1. * dEta;
      if (fGenSigmaEta>0.) dEta = AliPythiaRndm::GetPythiaRandom()->Gaus(dEta, fGenSigmaEta);
      dTheta = CovEtaToTheta(dEta);

      dPhi = dPhi + TMath::Pi();  
      if (fGenSigmaPhi>0.) dPhi = AliPythiaRndm::GetPythiaRandom()->Gaus(dPhi, fGenSigmaPhi);
      if (dPhi<0.) dPhi += TMath::TwoPi(); if (dPhi>TMath::TwoPi()) dPhi -= TMath::TwoPi();

      fPythia->Py1ent(iC, fGenPartonFlavor, fGenPartonEnergy, dTheta, dPhi);

      if (fUserDebug) cout << "iC = " << iC
                           << ", flavor = " << fGenPartonFlavor
                           << ", energy = " << fGenPartonEnergy
                           << ", theta = "  << dTheta
                           << ", phi = "    << dPhi << endl;
...
//
	...
	if (fQuench ==4){
	    ...
	    fImpact = bimp;
	    fPythia->Qpygin0();
            if (fUserDebug) cout << "Using QPYTHIA, k factor = " << fQhat << ", Imp param = " << bimp << endl;
	} 
//
...
//	printf("Calling hadronisation %d\n", fPythia->GetN());
	...
	fTrials++;
        if (fUserDebug) fPythia->Pylist(1);
...
//
	...
	if (np == 0) {
          if (fUserDebug) cout << "AliGenPythiaPair::Generate() : np == 0" << endl;
          continue;
        }
//
...
// No initial state partons
		if (ks==21) {
                  if (fUserDebug) cout << "AliGenPythiaPair::Generate() : ks == 21" << endl;
                  continue;
                }
//
...
// Kinematic seletion on final state heavy flavor mesons
		    ...
		    {
                        if (fUserDebug) cout << "AliGenPythiaPair::Generate() : parent not selected" << endl;
			continue;
		    }
		    pSelected[i] = 1;
...
// Kinematic seletion on decay products
		    ...
		    {
                        if (fUserDebug) cout << "AliGenPythiaPair::Generate() : child not selected" << endl;
			continue;
		    }
//
// Decay products 
...
//			printf("\n particle (decay)  %d %d %d", i, pSelected[i], kf);
		...
		if (!pSelected[i]) {
                  if (fUserDebug) cout << "AliGenPythiaPair::Generate() : no prarent" << endl;
                  continue;
                }
		// Count quarks only if you did not include fragmentation
		if (fFragmentation && kf <= 10) {
                  if (fUserDebug) cout << "AliGenPythiaPair::Generate() : fragmentation problem" << endl;
                  continue;
                }
...
//
		...
		for (i = 0; i<np; i++) {
		    if (!pSelected[i]) {
                      if (fUserDebug) cout << "AliGenPythiaPair::Generate() : parent not selected in particle loop" << endl;
                      continue;
                    }
		    TParticle *  iparticle = (TParticle *) fParticles.At(i);
		    kf = CheckPDGCode(iparticle->GetPdgCode());
...
Int_t  AliGenPythiaPair::GenerateMB()
{
//
...
//  converts from mm/c to s
  ...
  if ((fFragPhotonInCalo || fPi0InCalo || fEtaInCalo || fEleInCalo || fHadronInCalo || fDecayPhotonInCalo) && 
	...
	if (statusCode != 1) {
          if (fUserDebug) cout << "AliGenPythiaPair::GenerateMB() : status code != 1" << endl;
	  continue;
        }
	// eta cut
	if (fTriggerMultiplicityEta > 0 && TMath::Abs(iparticle->Eta()) > fTriggerMultiplicityEta) {
          if (fUserDebug) cout << "AliGenPythiaPair::GenerateMB() : trigger multi eta" << endl;
	  continue;
        }
	// pt cut
	if (iparticle->Pt() < fTriggerMultiplicityPtMin) {
          if (fUserDebug) cout << "AliGenPythiaPair::GenerateMB() : trigger multi pT" << endl;
	  continue;
        }

	TParticlePDG* pdgPart = iparticle->GetPDG();
	if (pdgPart && pdgPart->Charge() == 0) {
          if (fUserDebug) cout << "AliGenPythiaPair::GenerateMB() : charge == 0" << endl;
	  continue;
        }
	...
	for (i = 0; i < np; i++) {
	    TParticle *  iparticle = (TParticle *) fParticles.At(i);
	    kf = CheckPDGCode(iparticle->GetPdgCode());
	    if (kf != fTriggerParticle) {
              if (fUserDebug) cout << "AliGenPythiaPair::GenerateMB() : kf != fTriggerParticle" << endl;
              continue;
            }
	    if (iparticle->Pt() == 0.) {
              if (fUserDebug) cout << "AliGenPythiaPair::GenerateMB() : pT ==  0" << endl;
              continue;
            }
	    if (TMath::Abs(iparticle->Eta()) > fTriggerEta) continue;
	    if ( iparticle->Pt() > fTriggerMaxPt || iparticle->Pt() < fTriggerMinPt ) {
              if (fUserDebug) cout << "AliGenPythiaPair::GenerateMB() : pT selection" << endl;
              continue;
            }
	    triggered = kTRUE;
	    break;
	...
	if(fCutOnChild && TMath::Abs(pdg) == fPdgCodeParticleforAcceptanceCut) {
	  ...
	  if(mi<0) {
            if (fUserDebug) cout << "AliGenPythiaPair::GenerateMB() : mi < 0" << endl;
            continue;
          }
...
void AliGenPythiaPair::FinishRun()
{
...
void AliGenPythiaPair::AdjustWeights() const
{
...
void AliGenPythiaPair::SetNuclei(Int_t a1, Int_t a2, Int_t pdfset)
{
...
void AliGenPythiaPair::MakeHeader()
{
//
...
Bool_t AliGenPythiaPair::CheckTrigger(const TParticle* jet1, const TParticle* jet2)
{
...
Bool_t AliGenPythiaPair::CheckKinematicsOnChild(){
//
...
void  AliGenPythiaPair::LoadEvent(AliStack* stack, Int_t flag, Int_t reHadr)
{
  //
...
void  AliGenPythiaPair::LoadEvent(const TObjArray* stack, Int_t flag, Int_t reHadr)
{
  //
  // Load event into Pythia Common Block
  //
...
void AliGenPythiaPair::RecJetsUA1(Int_t& njets, Float_t jets[4][50])
{
//
...
void  AliGenPythiaPair::GetJets(Int_t& nJets, Int_t& nJetsTrig, Float_t jets[4][10])
{
//
...
void AliGenPythiaPair::GetSubEventTime()
{
...
Bool_t AliGenPythiaPair::IsInBarrel(Float_t eta) const
{
...
Bool_t AliGenPythiaPair::IsInEMCAL(Float_t phi, Float_t eta) const
{
...
Bool_t AliGenPythiaPair::IsInPHOS(Float_t phi, Float_t eta, Int_t iparticle)
{
...
void AliGenPythiaPair::RotatePhi(Bool_t& okdd)
{
...
Bool_t AliGenPythiaPair::CheckDiffraction()
{
...
Bool_t AliGenPythiaPair::GetWeightsDiffraction(Double_t M, Double_t &Mmin, Double_t &Mmax, 
...
Bool_t AliGenPythiaPair::IsFromHeavyFlavor(Int_t ipart)
{
...
Bool_t AliGenPythiaPair::CheckDetectorAcceptance(Float_t phi, Float_t eta, Int_t iparticle)
{
...
Bool_t AliGenPythiaPair::TriggerOnSelectedParticles(Int_t np)
{
  // Select events with fragmentation photon, decay photon, pi0, eta or other hadrons going to PHOS or EMCAL or central barrel,
  // implemented primaryly for kPyJets, but extended to any kind of process.
...
Double_t AliGenPythiaPair::CovEtaToTheta(Double_t dy)
{
//
//  AliGenPythiaPair::CovEtaToTheta
//

  Double_t dk = TMath::Exp(-1.*dy);
  Double_t dt = TMath::ATan(1.*dk);
  return (2.*dt);
}

//_____________________________________________________________________________
Double_t AliGenPythiaPair::CovThetaToEta(Double_t dt)
{
//
//  AliGenPythiaPair::CovThetaToEta
//

  Double_t dk = TMath::Tan(0.5*dt);
  Double_t dy = TMath::Log(1.0*dk);
  return (-1.*dy);
}
