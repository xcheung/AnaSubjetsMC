...
//
//    Pythia
    AliGenPythiaPair * gener = new AliGenPythiaPair(-1);
    ...
    gener->SetPtRange(0,5000.);

//  gener->SetUserDebug(kTRUE);
    gener->SetIsGenPartonPair(kTRUE);
//  gener->SetIsGenUnsignEta(kTRUE);

//  gener->SetIsGenFixedEta(kTRUE);
//  gener->SetGenPartonEtaRange(-1., 1.);
//  gener->SetGenSigmaEta(-1.);

//  gener->SetIsGenFixedPhi(kTRUE);
//  gener->SetGenPartonPhiRange(0., TMath::TwoPi());
//  gener->SetGenSigmaPhi(-1.);

    gener->SetGenPartonNumber(1);
    gener->SetGenPartonFlavor(kPartonFlv);
    gener->SetGenPartonEnergy(dPartonEn);
...
void ProcessEnvironmentVars()
{
    ...
    if (gSystem->Getenv("CONFIG_PARTONEN")) {
      dPartonEn = atof(gSystem->Getenv("CONFIG_PARTONEN"));
    }

    if (gSystem->Getenv("CONFIG_PARTONFL")) {
      kPartonFlv = atoi(gSystem->Getenv("CONFIG_PARTONFL"));
    }
