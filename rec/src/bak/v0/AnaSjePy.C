...
//=============================================================================
      ...
      for (int i=0; i<consti.size(); i++) {
        ...
        hJetTrkPt->Fill(dJetPt, dTrk); nTrk += 1.;
        if (dTrk>d1tk) { d2tk = d1tk; d1tk = dTrk; } else if (dTrk>d2tk) { d2tk = dTrk; }
      }

      hJetPtNtrk->Fill(dJetPt, nTrk);
      if (d1tk>0.) hJet1tkPt->Fill(dJetPt, d1tk);
      if (d2tk>0.) hJet2tkPt->Fill(dJetPt, d2tk);

      if ((d1tk>0.) && (d2tk>0.)) {
        double dtk = d1tk - d2tk;
        hJetPtDtk->Fill(dJetPt, dtk);
        hJetPtZtk->Fill(dJetPt, dtk/dJetPt);
      }
//=============================================================================
      ...
      for (int i=0; i<trimmdSj01.size(); i++) {
        ...
        hJetIsj01Pt->Fill(dJetPt, dIsj); nIsj += 1.;
        if (dIsj>d1sj) { d2sj = d1sj; d1sj = dIsj; } else if (dIsj>d2sj) { d2sj = dIsj; }
      }

      hJetPtNsj01->Fill(dJetPt, nIsj);
      if (d1sj>0.) hJet1sj01Pt->Fill(dJetPt, d1sj);
      if (d2sj>0.) hJet2sj01Pt->Fill(dJetPt, d2sj);

      if ((d1sj>0.) && (d2sj>0.)) {
        double dsj = d1sj - d2sj;
        hJetPtDsj01->Fill(dJetPt, dsj);
        hJetPtZsj01->Fill(dJetPt, dsj/dJetPt);
      }
//=============================================================================
      ...
      for (int i=0; i<trimmdSj02.size(); i++) {
        ...
        hJetIsj02Pt->Fill(dJetPt, dIsj); nIsj += 1.;
        if (dIsj>d1sj) { d2sj = d1sj; d1sj = dIsj; } else if (dIsj>d2sj) { d2sj = dIsj; }
      }

      hJetPtNsj02->Fill(dJetPt, nIsj);
      if (d1sj>0.) hJet1sj02Pt->Fill(dJetPt, d1sj);
      if (d2sj>0.) hJet2sj02Pt->Fill(dJetPt, d2sj);

      if ((d1sj>0.) && (d2sj>0.)) {
        double dsj = d1sj - d2sj;
        hJetPtDsj02->Fill(dJetPt, dsj);
        hJetPtZsj02->Fill(dJetPt, dsj/dJetPt);
      }
    }
//=============================================================================
