...
for dPtHat in \
  ...
  dPtHat_0334_INFN
do
  sDataPath="${DataPath}#${dPtHat}#hepmcs"
...
#
  ...
  echo "module load xmzhang/pdsf/HepMC/2.06.09"                          >> ${sTaskPath}/${ExeName}.sh
  echo "export PATH=${TopPath}/bin:\${PATH}"                             >> ${sTaskPath}/${ExeName}.sh
  echo "cd ${sTaskPath}"                                                 >> ${sTaskPath}/${ExeName}.sh
  echo "${ExeName}.exe \"${sDataPath}\" out_\${SGE_TASK_ID} ${EventWgt}" >> ${sTaskPath}/${ExeName}.sh
