...
//  std::vector<fastjet::PseudoJet> subtedJets = bkgSubtractor(includJets);
    ...
    for (int j=0; j<sortedJets.size(); j++) {
      ...
      if (j==0) hL1j->Fill(dJet, dNorm);
      if (j==1) hL2j->Fill(dJet, dNorm); if (j>1) continue;
//=============================================================================
      ...
      for (int i=0; i<trimmdSj.size(); i++) {
        ...
        if (j==0) { hL1jIsj->Fill(dJet, dIsj, dNorm); hL1jIsz->Fill(dJet, dIsj/dJet, dNorm); }
        if (j==1) { hL2jIsj->Fill(dJet, dIsj, dNorm); hL2jIsz->Fill(dJet, dIsj/dJet, dNorm); }
        if (dIsj>d1sj) { d2sj = d1sj; d1sj = dIsj; } else if (dIsj>d2sj) { d2sj = dIsj; }

        nIsj += 1.;
      }

      if (j==0) hL1jNsj->Fill(dJet, nIsj, dNorm);
      if (j==1) hL2jNsj->Fill(dJet, nIsj, dNorm);

      if (d1sj>0.) {
        if (j==0) { hL1j1sj->Fill(dJet, d1sj, dNorm); hL1j1sz->Fill(dJet, d1sj/dJet, dNorm); }
        if (j==1) { hL2j1sj->Fill(dJet, d1sj, dNorm); hL2j1sz->Fill(dJet, d1sj/dJet, dNorm); }
      }

      if (d2sj>0.) {
        if (j==0) { hL1j2sj->Fill(dJet, d2sj, dNorm); hL1j2sz->Fill(dJet, d2sj/dJet, dNorm); }
        if (j==1) { hL2j2sj->Fill(dJet, d2sj, dNorm); hL2j2sz->Fill(dJet, d2sj/dJet, dNorm); }
      }

      if ((d1sj>0.) && (d2sj>0.)) {
        double dsj = d1sj - d2sj;
        if (j==0) { hL1jDsj->Fill(dJet, dsj, dNorm); hL1jDsz->Fill(dJet, dsj/dJet, dNorm); }
        if (j==1) { hL2jDsj->Fill(dJet, dsj, dNorm); hL2jDsz->Fill(dJet, dsj/dJet, dNorm); }
      }
    }
//=============================================================================
