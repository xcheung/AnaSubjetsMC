const Int_t nJEL = 12;
const TString sJEL[] = {
  "dPtHat_0005_0011", "dPtHat_0011_0021", "dPtHat_0021_0036", "dPtHat_0036_0057",
  "dPtHat_0057_0084", "dPtHat_0084_0117", "dPtHat_0117_0152", "dPtHat_0152_0191",
  "dPtHat_0191_0234", "dPtHat_0234_0282", "dPtHat_0282_0334", "dPtHat_0334_INFN" };
...
TH1D *GetHistoJEL(const TString sHist, const TString sPath)
{
  ...
  for (Int_t i=0; i<nJEL; i++) {
    file = TFile::Open(Form("%s/AnalysisResults_%s.root",sPath.Data(),sJEL[i].Data()), "READ");
...
TH1D *GetHistoPy8(const TString sHist, const TString sPath)
{
  ...
  for (Int_t i=0; i<nPy8; i++) {
    file = TFile::Open(Form("%s/AnalysisResults_%s.root",sPath.Data(),sPy8[i].Data()), "READ");
...
TH1D *GetHistoQpy(const TString sHist, const TString sPath)
{
  ...
  for (Int_t i=0; i<nQpy; i++) {
    file = TFile::Open(Form("%s/AnalysisResults_%s.root",sPath.Data(),sQpy[i].Data()), "READ");
    Double_t dX = ((TProfile*)file->Get("hSigmaGen"))->GetBinContent(1);
    Double_t dW = ((TH1D*)file->Get("hWeightSum"))->GetBinContent(1);
    Double_t dWgt = dX / dW;
