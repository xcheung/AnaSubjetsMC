...
//=============================================================================
  ...
  TH2D *h5py8 = (TH2D*)file->Get("hJet2fr"); h5py8->SetName("h5py8"); h5py8->SetDirectory(0);
  file->Close();

  file = TFile::Open("data/mass/AnalysisOutputs_Dsm_qpy_JetR05_SjeR01.root", "READ");
  TH2D *h5qpy = (TH2D*)file->Get("hJet2fr"); h5qpy->SetName("h5qpy"); h5qpy->SetDirectory(0);
  file->Close();

  file = TFile::Open("data/mass/AnalysisOutputs_Dsm_med_JetR05_SjeR01.root", "READ");
  TH2D *h5med = (TH2D*)file->Get("hJet2fr"); h5med->SetName("h5med"); h5med->SetDirectory(0);
  file->Close();


  file = TFile::Open("data/mass/AnalysisOutputs_Dsm_py8_JetR04_SjeR01.root", "READ");
  TH2D *h4py8 = (TH2D*)file->Get("hJet2fr"); h4py8->SetName("h4py8"); h4py8->SetDirectory(0);
  file->Close();

  file = TFile::Open("data/mass/AnalysisOutputs_Dsm_qpy_JetR04_SjeR01.root", "READ");
  TH2D *h4qpy = (TH2D*)file->Get("hJet2fr"); h4qpy->SetName("h4qpy"); h4qpy->SetDirectory(0);
  file->Close();

  file = TFile::Open("data/mass/AnalysisOutputs_Dsm_med_JetR04_SjeR01.root", "READ");
  TH2D *h4med = (TH2D*)file->Get("hJet2fr"); h4med->SetName("h4med"); h4med->SetDirectory(0);
  file->Close();
//=============================================================================
...
//=============================================================================
  ...
  const TString stny = "#it{R}(sj^{2nd},jet)";
...
//=============================================================================
  ...
  can = MakeCanvas("Jet5py82frCorr"); can->SetGridx(); can->SetGridy(); can->SetLogz();
...
//=============================================================================
  ...
  can = MakeCanvas("Jet5qpy2frCorr"); can->SetGridx(); can->SetGridy(); can->SetLogz();
  can->SetTopMargin(0.1); can->SetRightMargin(0.1);
  hfm = can->DrawFrame(dflx, dfly, dfux, dfuy); SetupFrame(hfm, dlsx, dlsy, dtsx, dtsy, dtox, dtoy, stnx, stny);
  hfm->GetXaxis()->SetNdivisions(510);
  hfm->GetYaxis()->SetNdivisions(510);
...
//=============================================================================
  ...
  can = MakeCanvas("Jet5med2frCorr"); can->SetGridx(); can->SetGridy(); can->SetLogz();
  can->SetTopMargin(0.1); can->SetRightMargin(0.1);
  hfm = can->DrawFrame(dflx, dfly, dfux, dfuy); SetupFrame(hfm, dlsx, dlsy, dtsx, dtsy, dtox, dtoy, stnx, stny);
  hfm->GetXaxis()->SetNdivisions(510);
  hfm->GetYaxis()->SetNdivisions(510);
...
//=============================================================================
  ...
  can = MakeCanvas("Jet4py82frCorr"); can->SetGridx(); can->SetGridy(); can->SetLogz();
  can->SetTopMargin(0.1); can->SetRightMargin(0.1);
  hfm = can->DrawFrame(dflx, dfly, dfux, dfuy); SetupFrame(hfm, dlsx, dlsy, dtsx, dtsy, dtox, dtoy, stnx, stny);
  hfm->GetXaxis()->SetNdivisions(510);
  hfm->GetYaxis()->SetNdivisions(510);
...
//=============================================================================
  ...
  can = MakeCanvas("Jet4qpy2frCorr"); can->SetGridx(); can->SetGridy(); can->SetLogz();
  can->SetTopMargin(0.1); can->SetRightMargin(0.1);
  hfm = can->DrawFrame(dflx, dfly, dfux, dfuy); SetupFrame(hfm, dlsx, dlsy, dtsx, dtsy, dtox, dtoy, stnx, stny);
  hfm->GetXaxis()->SetNdivisions(510);
  hfm->GetYaxis()->SetNdivisions(510);
...
//=============================================================================
  ...
  can = MakeCanvas("Jet4med2frCorr"); can->SetGridx(); can->SetGridy(); can->SetLogz();
  can->SetTopMargin(0.1); can->SetRightMargin(0.1);
  hfm = can->DrawFrame(dflx, dfly, dfux, dfuy); SetupFrame(hfm, dlsx, dlsy, dtsx, dtsy, dtox, dtoy, stnx, stny);
  hfm->GetXaxis()->SetNdivisions(510);
  hfm->GetYaxis()->SetNdivisions(510);
