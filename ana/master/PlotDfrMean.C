...
//=============================================================================
  ...
  TH2D *h5py8 = (TH2D*)file->Get("hJetDfr"); h5py8->SetName("h5py8"); h5py8->SetDirectory(0);
  file->Close();

  file = TFile::Open("data/mass/AnalysisOutputs_Dsm_qpy_JetR05_SjeR01.root", "READ");
  TH2D *h5qpy = (TH2D*)file->Get("hJetDfr"); h5qpy->SetName("h5qpy"); h5qpy->SetDirectory(0);
  file->Close();

  file = TFile::Open("data/mass/AnalysisOutputs_Dsm_med_JetR05_SjeR01.root", "READ");
  TH2D *h5med = (TH2D*)file->Get("hJetDfr"); h5med->SetName("h5med"); h5med->SetDirectory(0);
  file->Close();


  file = TFile::Open("data/mass/AnalysisOutputs_Dsm_py8_JetR04_SjeR01.root", "READ");
  TH2D *h4py8 = (TH2D*)file->Get("hJetDfr"); h4py8->SetName("h4py8"); h4py8->SetDirectory(0);
  file->Close();

  file = TFile::Open("data/mass/AnalysisOutputs_Dsm_qpy_JetR04_SjeR01.root", "READ");
  TH2D *h4qpy = (TH2D*)file->Get("hJetDfr"); h4qpy->SetName("h4qpy"); h4qpy->SetDirectory(0);
  file->Close();

  file = TFile::Open("data/mass/AnalysisOutputs_Dsm_med_JetR04_SjeR01.root", "READ");
  TH2D *h4med = (TH2D*)file->Get("hJetDfr"); h4med->SetName("h4med"); h4med->SetDirectory(0);
  file->Close();
//=============================================================================
  ...
  TH1D *h5py8Mr = h5py8M->Rebin(nBin, "h5py8Mr", dBin); h5py8Mr->Scale(0.5); TH1D *h4py8Mr = h4py8M->Rebin(nBin, "h4py8Mr", dBin); h4py8Mr->Scale(0.5);
  TH1D *h5qpyMr = h5qpyM->Rebin(nBin, "h5qpyMr", dBin); h5qpyMr->Scale(0.5); TH1D *h4qpyMr = h4qpyM->Rebin(nBin, "h4qpyMr", dBin); h4qpyMr->Scale(0.5);
  TH1D *h5medMr = h5medM->Rebin(nBin, "h5medMr", dBin); h5medMr->Scale(0.5); TH1D *h4medMr = h4medM->Rebin(nBin, "h4medMr", dBin); h4medMr->Scale(0.5);
...
//=============================================================================
  ...
  const Float_t dfly = 0.1, dfuy = 0.5;
  ...
  const TString stny = "<#it{R}(sj^{1st},sj^{2nd})>";
...
//=============================================================================
  ...
  can = MakeCanvas("Jet5DfrMean"); can->SetGridx(); can->SetGridy();
...
//=============================================================================
  ...
  can = MakeCanvas("Jet4DfrMean"); can->SetGridx(); can->SetGridy();
  hfm = can->DrawFrame(dflx, dfly, dfux, dfuy); SetupFrame(hfm, dlsx, dlsy, dtsx, dtsy, dtox, dtoy, stnx, stny);
  hfm->GetXaxis()->SetNdivisions(510);
  hfm->GetYaxis()->SetNdivisions(510);
